...
- 적용 파일: `src/styles/tokens.css`(§7 그대로 옮김). GNB·공통 UI는 원본 글꼴을 사용하며 headnerve의 `--figma-*` 토큰은 쓰지 않는다. 상세 콘텐츠의 Pretendard·글자 크기·간격은 아래 사용자 지정 읽기 서식을 따른다.
...
| 안내 박스 `.commonBox` | `border-top: 1px --madi-primary`, `#f8f9fa`, 왼쪽 220px 제목 칸 `#e6eaed`, 본문 15px/300 | 원본 안내 박스 문법. 상세 하단 진료 안내는 아래의 `.clinic-guide` 규칙 사용 |
...
ROOT-ADMIN 팝업은 `site/popups.css`에서 공용 런타임을 마디 토큰으로 꾸민다. 고정 헤더보다 높은 z-index 1200, 화면 가장자리 여백 20px, 최소 44px 버튼을 사용한다. PC·모바일 이미지는 관리자 설정을 따르며 목록 첫 화면에서만 표시한다. CMS 홍보 배너 슬롯은 없다. 페이지 상단의 사진 배너는 유지한다. 계약은 [root-admin-integration.md](root-admin-integration.md)를 따른다.

## 커뮤니티 읽기 서식 (2026-09-22 사용자 지정)

GNB는 원본 사이트 문법을 유지한다. 목록·상세의 일반 링크는 기본 밑줄을 숨기고 hover·active·focus에서 220ms로 표시한다. 버튼·배지·태그·분류 탭·페이지 번호·목차 항목처럼 자체 배경이나 테두리가 있는 링크는 밑줄 없이 배경·테두리 색상으로 반응한다. 텍스트로 표시한 이전·다음·목록 링크의 밑줄은 유지한다. 카드에서 글을 여는 제목·카드 전체 링크는 밑줄 없이 면·글자색 변화로 반응한다. 카드 안의 글쓴이처럼 텍스트만 있는 별도 액션은 공통 hover·active·focus 밑줄을 유지하고, 카테고리 배지는 색상으로 반응한다. 목록에서 글쓴이는 이름만 표시하고 원장 소개 링크를 유지한다. 검색은 목록 도구 줄 안에서 펼치며 Next Form으로 검색 URL을 갱신한다. 검색 결과 수 옆의 `전체 글 보기`는 검색어와 페이지를 초기화한다.

블로그·후기·FAQ 질문 목록은 페이지당 10건이며, 한 페이지만 있어도 현재 번호를 표시한다. 빈 목록·조회 오류에는 페이지네이션을 표시하지 않는다. 페이지 이동은 검색·분류 조건을 유지하고, 카테고리 변경은 첫 페이지로 돌아간다. 블로그·후기 카드의 분류 이름은 선택 링크이며 FAQ 세부 질환의 질문 목록은 브레드크럼으로 상위 분류를 알리고 질문 성격 필터만 둔다. 진료 영역·세부 질환 탭 두 줄을 반복하지 않는다.

상세 본문은 AVCD 참고 글의 Pretendard, 720px 폭, 18px/1.7, 문단 아래 24px를 기준으로 한다. H2/H3/H4는 32/24/20px, 768px 미만에서는 본문 17px와 소제목 22/20/18px다. 상세 페이지 제목은 세 콘텐츠 모두 데스크톱 44px/700/1.3, 모바일 28px/600/1.3, 자간 -0.03em이다. 제목 아래 메타 정보는 12px 간격이며, 헤더 아래 여백은 42px다. 본문 H2 위 110px·아래 20px(모바일 16px), H3 위 32px·아래 12px, H4 위 24px·아래 8px를 사용한다. 후기 요약과 FAQ 질문·핵심 답변도 모바일 본문 17px/1.7을 따른다. 목록·강조·기울임·인용·코드·표는 `article-reading.css`에 모은다. 복사한 제목의 고정 크기 마크도 반응형 제목 위계를 따른다. 모바일에서는 문단 안에 저장된 고정 글자 크기·행간도 본문과 소제목을 따르며 색상·강조는 유지한다. 데스크톱의 일반 문단은 명시적 서식을 유지한다.

FAQ 상세는 제목·핵심 답변·상세 답변·병원 관점을 동일한 720px 기사 폭에 정렬한다. 본문만 좁게 가운데 배치해 섹션 제목과 시작선이 어긋나지 않게 한다.

목차는 데스크톱에서 헤더 아래에 고정하고 모바일은 접이식으로 고정한다. 모바일 목차는 닫힌 최소 높이 50px, 안쪽 여백 12px 16px이며 높이에 패딩을 포함한다. 펼친 목록은 제목 아래 10px 안쪽 여백으로 첫 선택 항목과 구분하고, hover 배경은 실제 hover가 가능한 장치에만 적용한다. 조상 `#subContainer`의 overflow는 clip을 사용해 sticky의 스크롤 기준이 화면으로 유지되게 한다.
...
블로그 행의 썸네일은 데스크톱에서 폭 11rem이며, 제목·설명·배지 묶음의 높이에 맞춰 위아래를 정렬한다. 이미지는 비율을 유지하며 프레임을 채우고 넘치는 부분만 가운데 기준으로 자른다. 모바일 세로 카드에서는 원본 비율 1155:634를 유지한다.

블로그 행의 오른쪽은 날짜 위·글쓴이 아래로 배치하고 이름은 일반 글자색을 사용한다. 카테고리는 작은 배지이며 hover·active·focus에서 배경·테두리 색상을 바꾸고 밑줄은 표시하지 않는다. 후기 상세도 제목·이미지·본문을 720px로 맞추고, 본문 이미지는 읽기 폭까지 표시한다. HTML·Tiptap 이미지는 클릭·키보드로 확대하며 2배 확대·Esc·닫기·포커스 복귀를 제공한다. 이미 링크가 있는 이미지는 원래 동작을 유지한다.

블로그·후기 상세의 카테고리 배지는 해당 분류 목록으로 연결한다. 블로그·후기 상세 하단은 공통 `ArticleNavigation`을 사용해 이전글·목록·다음글을 좌·중·우에 둔다. 호버 시 위치를 움직이지 않으며 색상·밑줄만 전환한다. 블로그는 같은 카테고리의 발행순으로 이동한다. 안내 문구 14px, 제목 16px/1.6이며 모바일에서도 세 칸을 유지하고 제목은 14px 두 줄 말줄임표로 표시한다.

블로그·후기 상세의 카테고리 배지는 제목 위 가운데에 둔다. FAQ 상세는 브레드크럼과 겹치는 배지를 두지 않고, 작성자 이름과 날짜만 표시한다. 작성자 설명과 날짜 앞 `답변일` 문구는 생략한다. 제목 아래 같은 줄의 왼쪽에는 작성자·날짜, 오른쪽에는 선호 출처 버튼을 배치한다. 좁은 화면에서는 왼쪽 메타 정보 안에서 줄을 나누고 버튼은 오른쪽을 유지한다.

블로그·후기·FAQ 상세 상단에는 공통 `PreferredSourceLink`를 표시한다. headnerve와 같은 Google 공식 딥링크 `https://www.google.com/preferences/source?q=gwangju2020blog.madiclinic.co.kr`를 새 창으로 열고 사용자가 출처를 선택한다. `q`는 `siteOrigin`의 호스트명이며 상세 경로나 다른 병원 도메인을 넣지 않는다. 버튼은 Google G 아이콘과 `선호 출처로 추가`, 최소 높이 44px를 사용한다. 참고: https://developers.google.com/search/docs/appearance/preferred-sources?hl=ko

후기 상세는 진료 분야·작성자·날짜·선호 출처 버튼을 하나의 `review-detail__metadata` flex 영역에 가로로 펼친다. 환자·치료 기간이 있으면 같은 영역에 표시하고, 좁은 화면에서만 줄바꿈한다. FAQ 바깥 섹션 제목과 간격은 본문 H2 위계를, 질문·핵심 답변 카드 제목은 H3 위계를 따른다.

상세 하단 진료 안내는 본문과 60px(980px 이하 40px) 간격을 둔다. 원본 병원의 `iconCommonInfoTitle.png`는 병원명 위에 작게 놓고, 병원 정보·진료시간·예약 동작은 하나의 옅은 회색 면과 16px 모서리 안에 한 열로 정렬한다. 모바일에서는 아이콘·병원명·주소만 가운데 정렬하고 진료시간과 예약 안내는 왼쪽 정렬한다. 390px에서 예약 안내 문구는 두 줄이다. 원본 병원 `clinic01.html`의 네이버 예약 아이콘·초록색 `#46b85c`, 전화 아이콘·보라색 `#5971b0`는 유지한다. 버튼은 높이 56px, 아이콘과 문구를 한 묶음으로 가운데 정렬하며 두 열(520px 이하 한 열)을 사용한다. 안내 문구 아래 28px(모바일 20px), 버튼 사이 24px(좁은 모바일 12px)를 확보한다. 휴진 안내는 진료시간 바로 아래에 둔다. 문구는 `네이버 예약 바로가기`, `지금 바로 전화걸기`이고 카카오 문의는 두지 않는다. 추적 ID와 위치 식별자는 유지한다.

블로그 목록 마지막 행에는 아래 구분선을 두지 않는다. 본문 내부 링크는 작은 문서 아이콘·차분한 파란색·옅은 밑줄과 작은 `관련 글` 주석으로 구분하며, hover·focus 때만 배경을 살짝 강조한다. 외부 링크·목차·이미지 링크에는 이 표시를 붙이지 않는다. 스타일은 `internal-links.css`에 둔다. 블로그·후기 상세 제목과 본문 섹션, FAQ 상세 섹션의 장식 가로선은 여백과 배경의 명도 차이로 대체한다. FAQ 질문 목록은 항목 사이에만 선을 두며, 페이지 번호 위와 사이드바 제목 아래의 선은 없앤다. FAQ 세부 질문 화면의 상단 성격 필터는 오른쪽 목차와 중복되므로 제거하고, 데스크톱에서는 목차 링크로 분류를 고른다. 모바일은 질문 그룹 제목과 목록을 이어서 읽는다. 표 셀·브레드크럼·입력창처럼 구획이 필요한 선은 유지한다. 목차 앵커는 고정 헤더 여백을 유지하면서 부드럽게 이동하고, 동작 줄이기 설정에서는 즉시 이동한다. 페이지 간 이동은 Next.js의 기본 즉시 이동을 유지한다.
