...
| 컴포넌트 | 원본 규칙 | 블로그 사용처 |
|---|---|---|
| 페이지 골격 `.madi-page-frame` | 상단 여백 `--madi-header-h`, 사진 배너 없음 | 2026-09-23 사용자 요청으로 서브 배너를 제거. 원본 사진·치수는 추출 기록으로만 보관 |
| 브레드크럼 `.whereIsLine ul.whereIs` | 60px 띠, `#f9f9f9`, 상하 `#eee` 선, 홈 아이콘 40px 칸, 상위 항목 140px, 마지막 제목은 남은 너비(최소 240px)와 한 줄 말줄임표·전체 제목 tooltip, `#777` | 커뮤니티 메뉴 아래. `MadiBreadcrumb`에 적용 |
...
`site/community.css`는 원본 헤더와 분리된 본문 공통 규칙이다. 제목과 목록의 좌우 시작선을 맞추고, 바깥 `cBox`가 상단 간격을 한 번만 담당한다(데스크톱 60px, 980px 이하 40px). 모바일 본문 좌우 여백은 20px다.

목록 화면은 메뉴·현재 위치에서 분류를 알리므로 본문의 반복 제목·소개 문구를 두지 않는다. 접근성을 위한 페이지 제목과 본문 이름은 유지한다. 블로그·후기는 검색과 목록, FAQ는 분류와 질문으로 바로 시작한다. 상세 글 제목과 CMS 분류 카드의 설명은 유지한다. 블로그·후기 장문은 최대 720px 읽기 폭을 사용한다. CMS 미설정·조회 실패·검색 결과 없음은 구분해서 한 번만 안내한다.

ROOT-ADMIN 팝업은 `site/popups.css`에서 공용 런타임을 마디 토큰으로 꾸민다. 고정 헤더보다 높은 z-index 1200, 화면 가장자리 여백 20px, 최소 44px 버튼을 사용한다. PC·모바일 이미지는 관리자 설정을 따르며 목록 첫 화면에서만 표시한다. 홍보 배너 슬롯은 없다. 계약은 [root-admin-integration.md](root-admin-integration.md)를 따른다.
...
블로그·후기·FAQ 상세의 카테고리 배지는 제목 위 가운데에 둔다. 제목 아래 같은 줄의 왼쪽에는 작성자·날짜, 오른쪽에는 선호 출처 버튼을 배치한다. 좁은 화면에서는 왼쪽 메타 정보 안에서 줄을 나누고 버튼은 오른쪽을 유지한다.
...
후기 환자·진료 분야·치료 기간은 작성자·날짜 아래 정보 줄에 둔다. FAQ 바깥 섹션 제목과 간격은 본문 H2 위계를, 질문·핵심 답변 카드 제목은 H3 위계를 따른다.

상세 하단 진료 안내는 본문과 60px(980px 이하 40px) 간격을 둔다. 원본 병원 `clinic01.html`의 네이버 예약 아이콘·초록색 `#46b85c`, 전화 아이콘·보라색 `#5971b0`와 60px 높이·30px 모서리를 사용한다. 버튼 문구는 `네이버 예약 바로가기`, `지금 바로 전화걸기`이며 카카오 문의는 이 안내에서 제외한다. 버튼은 안내 박스 전체 폭에 두 열, 660px 이하에서는 한 열로 배치한다.

블로그 목록 마지막 행에는 아래 구분선을 두지 않는다. 본문의 내부 글 링크는 차분한 파란색·옅은 밑줄·작은 오른쪽 화살표로 구분하며, hover·focus 때만 강조한다. 목차 앵커는 고정 헤더 여백을 유지하면서 부드럽게 이동하고, 동작 줄이기 설정에서는 즉시 이동한다. 페이지 간 이동은 Next.js의 기본 즉시 이동을 유지한다.
