...
목차는 데스크톱에서 헤더 아래에 고정하고 모바일은 접이식으로 고정한다. 모바일 목차는 닫힌 높이 52px, 안쪽 여백 12px 16px이며 높이에 패딩을 포함한다. 조상 `#subContainer`의 overflow는 clip을 사용해 sticky의 스크롤 기준이 화면으로 유지되게 한다.
...
상세 하단 진료 안내는 본문과 60px(980px 이하 40px) 간격을 둔다. 큰 그림 칸 대신 병원 정보·진료시간·예약 동작을 하나의 옅은 회색 면과 16px 모서리 안에 정렬한다. 원본 병원 `clinic01.html`의 네이버 예약 아이콘·초록색 `#46b85c`, 전화 아이콘·보라색 `#5971b0`는 유지한다. 버튼은 높이 56px, 아이콘과 문구를 한 묶음으로 가운데 정렬하며 두 열(520px 이하 한 열)을 사용한다. 안내 문구 아래 28px, 버튼 사이 24px(모바일 16px)를 확보한다. 휴진 안내는 진료시간 바로 아래에 둔다. 문구는 `네이버 예약 바로가기`, `지금 바로 전화걸기`이고 카카오 문의는 두지 않는다. 추적 ID와 위치 식별자는 유지한다.

블로그 목록 마지막 행에는 아래 구분선을 두지 않는다. 본문 내부 링크는 작은 문서 아이콘·차분한 파란색·옅은 밑줄과 작은 `관련 글` 주석으로 구분하며, hover·focus 때만 배경을 살짝 강조한다. 외부 링크·목차·이미지 링크에는 이 표시를 붙이지 않는다. 스타일은 `internal-links.css`에 둔다. 상세 제목 아래와 FAQ 핵심 답변 위의 파란 가로선은 제거하고 여백과 배경의 명도 차이로 구분한다. 목차 앵커는 고정 헤더 여백을 유지하면서 부드럽게 이동하고, 동작 줄이기 설정에서는 즉시 이동한다. 페이지 간 이동은 Next.js의 기본 즉시 이동을 유지한다.
