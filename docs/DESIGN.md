...
| 브레드크럼 `.whereIsLine ul.whereIs` | 60px 띠, `#f9f9f9`, 상하 `#eee` 선, 홈 아이콘 40px 칸, 항목 140px, 마지막 `#777` | 배너 바로 아래. headnerve `SiteBreadcrumb` 마크업에 이 스타일 |
...
GNB는 원본 사이트 문법을 유지한다. 목록·상세의 일반 링크는 기본 밑줄을 숨기고 hover·active·focus에서 220ms로 표시한다. 카드 안의 링크와 카드 전체 링크는 밑줄 없이 면·글자색 변화로 반응한다. 목록에서 글쓴이는 이름만 표시하고 원장 소개 링크를 유지한다. 검색은 목록 도구 줄 안에서 펼치며 Next Form으로 검색 URL을 갱신한다. 검색 결과 수 옆의 `전체 글 보기`는 검색어와 페이지를 초기화한다.
