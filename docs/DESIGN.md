...
목록 제목은 페이지당 한 번 표시하고 검색·건수·분류는 도구 영역에 둔다. 본문 제목의 반응형 크기 28px·22px는 원본 `patterns.css`의 섹션 제목 규칙을 따른 예외이며 새 전역 토큰으로 확장하지 않는다. 블로그·후기 장문은 최대 720px 읽기 폭을 사용한다. CMS 미설정·조회 실패·검색 결과 없음은 구분해서 한 번만 안내한다.
