...
2026-09-23 운영 확인: 공개 사이트의 검색 버튼 클릭 비콘이 `/v1/collect`로 1회
전송됐고, ROOT-ADMIN **성과 분석 → 페이지·문의**에서 페이지별 조회·버튼 클릭과
**유입 경로에서 버튼 클릭까지** 표가 집계됐다. 수집 이전 기록은 소급 복원할 수 없으며,
클릭은 실제 전화 연결이나 예약 완료를 뜻하지 않는다. GA4·GSC는 별도 연결 항목이다.
